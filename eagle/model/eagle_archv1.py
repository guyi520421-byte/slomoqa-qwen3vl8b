...
class EagleMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.config.mm_use_4_vision_tokens = mm_use_4_vision_tokens
...
class EagleMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, grid_thw):
    ...
    def reshape_2x2_image_features(self, image_features):
        ...
        # Check if P is odd
        if patch_size % 2 != 0:
            # Reshape to (B, patch_size, patch_size, D)
            image_features = image_features.reshape(B, patch_size, patch_size, D)
            
            # Create zero padding for the left side and top
            zero_padding_left = torch.zeros((B, patch_size, 1, D), device=image_features.device, dtype=image_features.dtype)
            zero_padding_top = torch.zeros((B, 1, patch_size + 1, D), device=image_features.device, dtype=image_features.dtype)
            
            # Concatenate zero padding to the left side
            image_features = torch.cat((zero_padding_left, image_features), dim=2)
            
            # Concatenate zero padding to the top
            image_features = torch.cat((zero_padding_top, image_features), dim=1)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, image_sizes=None, grid_thw=None
    ):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(concat_images, grid_thw)  # [B*T, P, D]
            if getattr(self.config, 'mm_use_4_vision_tokens', True) and grid_thw is None:
                image_features = self.reshape_2x2_image_features(image_features)
            split_sizes = [image.shape[0] for image in images]
            
            if getattr(self.config, 'mm_vision_select_feature', 'list') == 'list':
                image_features = [torch.split(image_feature, split_sizes, dim=0) for image_feature in image_features]
            else:
                image_features = torch.split(image_features, split_sizes, dim=0)
                image_features = [x.to(self.device) for x in image_features] 
                
        else:
            image_features = self.encode_images(images, grid_thw)  # [B, 576, 2048]
            if getattr(self.config, 'mm_use_4_vision_tokens', True) and grid_thw is None:
                image_features = self.reshape_2x2_image_features(image_features)  # [B*T, P/4, 4*D]
                
            if getattr(self.config, 'mm_vision_select_feature', 'list') == 'list':
                split_sizes = [1 for _ in images]
                image_features = [torch.split(image_feature, split_sizes, dim=0) for image_feature in image_features]
        ...
        if grid_thw is not None:
            N, _ = grid_thw.shape
            reduce_factor = 4 if getattr(self.config, 'mm_use_4_vision_tokens', True) else 1
            split_sizes = [torch.prod(grid_thw[i]).item() // reduce_factor for i in range(N)]
            assert sum(split_sizes) == image_features.shape[0], "The sum of split sizes must equal the first dimension of image_features"
